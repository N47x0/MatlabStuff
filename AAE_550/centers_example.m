%  MoC example to solve in-class example from AAE 550 slide 30, class 17; 
...
% design variables:
x0 = [400; 600];   % initial design point 
...
while (((abs(f_last - f) >= epsilon_f) | (max(g) >= epsilon_g)) ...
    ...
    [f,gradf] = Asses4_fun(x);
    f   % no semi-colon to obtain output
    [g, h, gradg, gradh] = Asses4_con(x);
